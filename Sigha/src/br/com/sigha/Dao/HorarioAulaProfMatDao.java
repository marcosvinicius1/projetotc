    ...
    public List<HorarioAulaProfMatBeans> RetornaHorarioProfMat(String anoletivo, Date datageracao) throws SQLException {
        try (Connection conexao = new ConexaoBanco().getConnect()) {
            String sql = "select h.id,s.nome curso,h.periodo,H.inicio,h.termino,"
                    + "p.nome professorsegunda,m.nome materiasegunda,"
                    + "pt.nome professorterca,mt.nome materiaterca,"
                    + "pq.nome professorquarta,mq.nome materiaquarta,"
                    + "pqu.nome professorquinta,mqu.nome materiaquinta,"
                    + "ps.nome professorsexta,ms.nome materiasexta,"
                    + "psb.nome professorsabado,msb.nome materiasabado,"
